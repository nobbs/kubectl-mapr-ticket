...
func (l *Lister) List() ([]apiVolume.Volume, error) {
...
func (l *Lister) getVolumes() error {
	...
	l.volumes = make([]apiVolume.Volume, 0, len(pvs.Items))

	for i := range pvs.Items {
		l.volumes = append(l.volumes, *apiVolume.NewVolume(&pvs.Items[i]))
	}
...
func (l *Lister) filterVolumesToMaprCSI() *Lister {
	var filtered []apiVolume.Volume

	for _, volume := range l.volumes {
		if IsMaprCSIBased(volume.Volume) {
...
func (l *Lister) filterVolumeUsesTicket() *Lister {
	...
	for _, volume := range l.volumes {
		if l.TicketUsesSecret(volume.Volume, &coreV1.SecretReference{
			Name:      l.secretName,
