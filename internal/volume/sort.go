...
func sortByName(volumes []util.Volume) {
	sort.Slice(volumes, func(i, j int) bool {
		return volumes[i].Name() < volumes[j].Name()
	})
}

func sortBySecretNamespace(volumes []util.Volume) {
	sort.Slice(volumes, func(i, j int) bool {
		return volumes[i].SecretNamespace() < volumes[j].SecretNamespace()
	})
}

func sortBySecretName(volumes []util.Volume) {
	sort.Slice(volumes, func(i, j int) bool {
		return volumes[i].SecretName() < volumes[j].SecretName()
	})
}

func sortByClaimNamespace(volumes []util.Volume) {
	sort.Slice(volumes, func(i, j int) bool {
		return volumes[i].ClaimNamespace() < volumes[j].ClaimNamespace()
	})
}

func sortByClaimName(volumes []util.Volume) {
	sort.Slice(volumes, func(i, j int) bool {
		return volumes[i].ClaimName() < volumes[j].ClaimName()
	})
}

func sortByVolumePath(volumes []util.Volume) {
	sort.Slice(volumes, func(i, j int) bool {
		return volumes[i].VolumePath() < volumes[j].VolumePath()
	})
}

func sortByVolumeHandle(volumes []util.Volume) {
	sort.Slice(volumes, func(i, j int) bool {
		return volumes[i].VolumeHandle() < volumes[j].VolumeHandle()
	})
}

func sortByExpiryTime(volumes []util.Volume) {
	sort.Slice(volumes, func(i, j int) bool {
...
func sortByAge(volumes []util.Volume) {
	sort.Slice(volumes, func(i, j int) bool {
		if volumes[i].Ticket == nil {
			return true
		} else if volumes[j].Ticket == nil {
		...
		return volumes[i].PV.CreationTimestamp.Time.Before(volumes[j].PV.CreationTimestamp.Time)
	})
}
